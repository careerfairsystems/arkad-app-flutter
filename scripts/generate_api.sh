...
GENERATOR_VERSION="7.10.0"  # Match CI version
...
download_generator() {
    ...
    if ! curl -fL --retry 2 --retry-delay 5 --max-time 120 "$url" -o "$JAR_FILE"; then
        log_error "Failed to download OpenAPI Generator CLI"
        exit 1
    fi
}
...
generate_api() {
    ...
    if ! java -jar "$JAR_FILE" generate \
        ...
        -g dart-dio \
        -o "$OUTPUT_DIR" \
        --additional-properties=pubName=arkad_api,pubVersion=1.0.0,pubDescription="Generated API client",nullSafe=true; then
